...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: listRes.map(({ target: item }) => {
            ...
            switch (type) {
                case 'answer':
                    title = `${item.question.title}-${item.author.name}的回答`;
                    if (titleContent) {
                        title += `：${item.excerpt}`;
                    }
